...
3 节点信息
master: 172.17.128.171
node1: 172.17.128.172
node2: 172.17.128.173

## 在 3 节点上设置 host, 编辑 `/etc/hosts`
```
172.17.128.171  master
172.17.128.172  node1
172.17.128.173  node2
```
...
--apiserver-cert-extra-sans: 用于提供证书的API服务器的ip。可以是IP地址和DNS名称，可选附加
...
## node 节点加入集群的命令
```
kubeadm join 172.17.128.171:6443 --token jzplod.ongibewq4iz588a3     --discovery-token-ca-cert-hash sha256:ae794db8e09e7a420e651a0b646da29fab6d12857d26ffb8eacc47f961b1fd53
```
此命令需要到 node 上去执行
...
本地访问
```
http://<master-ip>:<master-port>/api/v1/namespaces/kubernetes-dashboard/services/https:kubernetes-dashboard:/proxy/#/login
```
如果是远程访问，记，目前 chrome 浏览器不支持，因为证书问题，可以用 Firefox 浏览器访问
...
## 快速将 deployment 暴露为一个服务供集群外访问
```
kubectl expose deployment my-go-app --type=NodePort --name=go-app-svc --target-port=3000 service/go-app-svc exposed

kubectl expose deployment my-go-app --port=3000 --target-port=3000
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: dashboard-metrics-scraper
          image: kubernetesui/metrics-scraper:latest
          ports:
            - containerPort: 8000
              protocol: TCP
          livenessProbe:
